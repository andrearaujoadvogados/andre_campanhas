...
### 1.4 Recebimento de respostas

**O SES não emite evento de resposta.** Os destinos de evento do Configuration Set cobrem apenas o que acontece com a mensagem que sai — `Send`, `Delivery`, `Open`, `Click`, `Bounce`, `Complaint`, `Reject`, `RenderingFailure`, `DeliveryDelay`. Uma resposta é um e-mail novo, e saber que ela existiu exige **receber** o e-mail de volta.

O desenho tem quatro decisões, e cada uma existe por um motivo específico.

**Subdomínio próprio, não o domínio de envio.** O MX vai em `respostas.mail.andrearaujoadvogados.com.br`, um nome novo. O e-mail que o escritório já usa não é tocado — nenhum registro existente muda.

**A campanha viaja no endereço.** O `Reply-To:` das campanhas passa a ser `resposta+<campaignId>@respostas.mail…`. O `To:` de uma resposta é literalmente esse endereço, então a marca sobrevive a encaminhamento e a cliente que não preenche cabeçalho de thread. Só a campanha cabe: a parte local de um endereço tem 64 caracteres (RFC 5321) e `campanha.envio` passaria disso. Não precisa caber — o `sendId` é `hash(campanha, contato)`, então sabendo a campanha (do endereço) e o contato (do remetente da resposta) ele se recalcula, e o envio é encontrado por `GetItem` direto, sem índice novo.

**`In-Reply-To` é a rede, não o caminho principal.** Quando o contato responde de um endereço que não está na base, a busca por remetente não acha ninguém — mas o cabeçalho ainda carrega o Message-ID do SES, que o GSI4 já indexa. Os dois caminhos convivem.

**Toda resposta é encaminhada para a caixa do escritório.** É a condição para o resto ser aceitável: ligar o rastreamento troca o `Reply-To:` das campanhas por um endereço nosso, e sem o encaminhamento a comunicação do cliente morreria numa caixa que ninguém abre. O encaminhamento é uma mensagem nova, assinada por nós, com a original **anexada intacta** em `message/rfc822` — repassar a mensagem crua faria SPF e DMARC do domínio do remetente reprovarem, e reescrever o `From:` invalidaria a assinatura DKIM dela sem aviso.

**O interruptor é a variável `EMAIL_RESPOSTAS`.** Sem ela, nem a regra de recebimento é criada nem o `Reply-To:` muda: o código sobe e fica dormente até o MX estar publicado. Também é o que evita conflito entre ambientes — o SES admite um conjunto de regras de recebimento ativo por região, e dev e prod dividem a conta.

**O que fica pendente de operação:** publicar o MX (`10 inbound-smtp.us-east-2.amazonaws.com`) e ativar o conjunto de regras, que não tem recurso CloudFormation e exige `aws ses set-active-receipt-rule-set`.
...
| 8. Relatórios           | Contadores pré-agregados por campanha (CQRS-lite) + série diária de engajamento (`SERIE#<dia>` no item group da campanha, incrementada no processamento do evento) + carimbos de primeira abertura/clique/resposta no próprio envio. O painel agrega por ids explícitos (`/relatorios/resumo`, `/serie`, `/desempenho`) — nunca por Scan. A série acumula a partir da implantação; eventos anteriores não são re-agregados                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 11. Respostas           | Regra de recebimento do SES em subdomínio próprio → S3 + `reply-receiver` → fila de eventos → `registrarResposta`, que correlaciona pelo endereço marcado (campanha + remetente ⇒ `sendId` determinístico) ou por `In-Reply-To`. Contador por **e-mail respondido**, não por mensagem recebida; a lista de quem respondeu sai do carimbo no próprio envio. Toda resposta é encaminhada para a caixa do escritório (seção 1.4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 12. Boletim automático  | Fontes configuráveis no painel (site + instrução em texto livre do que coletar). Worker `boletim-builder` (sob demanda e pelas rotinas de envio automático): busca cada página (guarda de SSRF no domínio, revalidada na coleta), reduz a texto, extrai notícias via **Gemini** (nível gratuito do AI Studio; provedor atrás do port `ExtratorPorIa` — trocar de IA é um adaptador; cadeia de modelos configurável em `MODELOS_GEMINI`, adaptativa por fonte, com prazo interno menor que o teto da Lambda), valida a resposta campo a campo, faz uma **passada editorial** (segunda chamada à IA sobre o material já validado: escolhe o destaque, escreve título, abertura, leitura prática e o "no radar"; lê a matéria do destaque por inteiro para desenvolvê-la; falha aqui degrada para a edição padrão no mesmo layout, nunca derruba o envio) e monta o design com as MESMAS fábricas do painel (`@emailmkt/criador`), compilado com `mjml` Node (mesma major do `mjml-browser`). O layout é o da edição de referência do escritório: faixa vinho com o logo claro, card de destaque, "O que isso significa para você", "Também nestas semanas", "No radar", assinatura e rodapé claro. O resultado é um **modelo** na categoria Boletim — o disparo continua humano, pelo assistente, exceto nas rotinas de envio automático. Sem novidade nas fontes, sai uma **edição de retrospectiva** (segunda passada da IA pedindo o mais relevante e mais lido; senão, o acervo das edições anteriores), avisada ao leitor no próprio e-mail. Todo e-mail leva o logo do escritório no topo, servido pelo painel. Conteúdo de página e resposta de IA são tratados como não confiáveis: prompt delimitado contra injeção, tudo escapado na montagem, links restritos a http(s). Falha por fonte vira aviso, nunca derruba o lote |
...
**Boletim automatizado — a fundação já existe.** A rotina periódica que pesquisará as notícias e disparará o boletim monta o e-mail chamando as fábricas de `@emailmkt/criador` (`criarLinhaDestaque`, `criarLinhaNoticia`, `criarLinhaAbertura`, `criarLinhaRadar`…) e compila com `compileDesignToMjml` — o mesmo código do painel, extraído para um pacote puro (sem DOM) exatamente para um worker poder importá-lo. O que falta para a rotina: a fonte das notícias (pesquisa na web), o pacote `mjml` do Node para MJML→HTML no worker (o painel usa `mjml-browser`; o MJML intermediário é idêntico), e o agendamento (EventBridge Scheduler, já usado pelas campanhas). O teste "caminho da automação" em `criador-boletim.test.ts` fixa o contrato das fábricas. A convenção que liga tipo de campanha a modelo é `categoria do modelo = nome do tipo` — a rotina cria a campanha com `tipoEmailId` do tipo "Boletim" e o modelo da categoria correspondente.
