...
| 12. Boletim automático  | Fontes configuráveis no painel (site + instrução em texto livre do que coletar). Worker `boletim-builder` (segunda, 8h, e sob demanda): busca cada página (guarda de SSRF no domínio, revalidada na coleta), reduz a texto, extrai notícias via **Gemini** (nível gratuito do AI Studio; provedor atrás do port `ExtratorPorIa` — trocar de IA é um adaptador), valida a resposta campo a campo e monta o design com as MESMAS fábricas do painel (`@emailmkt/criador`), compilado com `mjml` Node (mesma major do `mjml-browser`). O resultado é um **modelo** na categoria Boletim — o disparo continua humano, pelo assistente. Conteúdo de página e resposta de IA são tratados como não confiáveis: prompt delimitado contra injeção, tudo escapado na montagem, links restritos a http(s). Falha por fonte vira aviso, nunca derruba o lote |
