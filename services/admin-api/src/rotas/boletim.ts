...
import { fonteId as novoFonteId, validarUrlDeFonte, type FonteBoletim } from '@emailmkt/core';
 ...
 * segundo plano. O resultado aparece como modelo novo na categoria Boletim;
 * a resposta diz exatamente isso para a tela repetir ao usuário.
 */
rotasBoletim.post('/gerar', async (c) => {
  ...
  await deps.geradorBoletim.gerarAgora();
  await deps.auditoria.registrar({
    tenantId: usuario.tenantId,
    userId: usuario.userId,
    acao: 'CRIOU',
  ...
  return c.json(
    {
      iniciado: true,
      message:
        'Geração iniciada. Em um ou dois minutos o boletim aparece em Modelos, na categoria Boletim.',
    },
    202,
  );
});
